...
fn registry_instruction_shapes_are_decoded() {
    assert!(matches!(
        stockstream::instruction::StockStreamInstruction::decode(&[19]),
        Ok(stockstream::instruction::StockStreamInstruction::InitializeExchange)
    ));
    let mut data = [0u8; 33];
    data[0] = 20;
    data[1..].fill(7);
    assert!(
        matches!(stockstream::instruction::StockStreamInstruction::decode(&data), Ok(stockstream::instruction::StockStreamInstruction::RegisterStockInstrument { instrument_id }) if instrument_id == [7; 32])
    );
}
