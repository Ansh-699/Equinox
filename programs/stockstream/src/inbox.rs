...
pub fn deposit_to_inbox_v3(program_id: &Address, accounts: &mut [AccountView], amount: u64) -> ProgramResult {
    ...
    let mint = Address::new_from_array(
        unsafe { accounts[0].borrow_unchecked() }[76..108].try_into().map_err(|_| StockStreamError::InvalidMarketLayout)?,
    );
    ...
    {
        let source = TokenAccount::from_account_view(&accounts[3]).map_err(|_| StockStreamError::CustodyViolation)?;
        let vault = TokenAccount::from_account_view(&accounts[4]).map_err(|_| StockStreamError::CustodyViolation)?;
...
pub fn claim_inbox_deposit_v3(program_id: &Address, accounts: &mut [AccountView], seat_index: u16) -> ProgramResult {
    ...
    let amount = total.checked_sub(credited).filter(|delta| *delta > 0).ok_or(StockStreamError::InvalidInstruction)?;
...
pub fn claim_withdrawal_v3(program_id: &Address, accounts: &mut [AccountView], seat_index: u16) -> ProgramResult {
    ...
    let amount = requested.checked_sub(paid).filter(|delta| *delta > 0).ok_or(StockStreamError::InvalidInstruction)?;
    {
        let destination = TokenAccount::from_account_view(&accounts[3]).map_err(|_| StockStreamError::CustodyViolation)?;
        let vault = TokenAccount::from_account_view(&accounts[4]).map_err(|_| StockStreamError::CustodyViolation)?;
