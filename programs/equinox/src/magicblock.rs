...
//!
//! Equinox is a `no_std`, no-heap Pinocchio 0.11.2 program. The official
...
//!
//! The delegated hot cluster is the market PDA plus every other Equinox
...
fn validate_cluster_member(
    ...
    if !account.owned_by(program_id) {
        return Err(custom(EquinoxError::MagicBlockInvalidAccount));
    }
    match account.data_len() {
        SETTLEMENT_SCRATCH_LEN => {
            ...
            if header.status != ScratchStatus::Empty as u8 {
                return Err(custom(EquinoxError::MagicBlockScratchNotEmpty));
            }
        ...
        _ => Err(custom(EquinoxError::MagicBlockInvalidAccount)),
    }
}
...
/// 4. `[WRITE]`          market delegate buffer PDA (`["buffer", market]`, Equinox-owned)
...
/// 9. `[]`               Equinox's own executable program account (the CPI's `owner_program`)
...
pub fn delegate_market(
    ...
    // `Restored` here).
    {
        ...
        {
            return Err(custom(EquinoxError::MagicBlockAlreadyDelegated));
        }
    }
    ...
    //      into it, zero the account, reassign Equinox -> System ->
...
/// 1. Create the buffer PDA (`["buffer", account]`, Equinox-owned) sized
...
/// 3. Reassign the account: Equinox -> System Program (direct, zeroed
...
/// 5. The buffer is still Equinox-owned (the delegation program never
...
fn delegate_single_account(
    ...
    // 3. Reassign: Equinox -> System Program (direct, zeroed data), then
...
) -> Result<usize, EquinoxError> {
    if seeds_payload.len() > MAX_SEEDS_PAYLOAD_LEN {
        return Err(EquinoxError::MagicBlockInvalidAccount);
    }
    out.fill(0);
    out[0..8].copy_from_slice(&DELEGATE_DISCRIMINATOR.to_le_bytes());
    out[8..12].copy_from_slice(&COMMIT_INTERVAL_MS.to_le_bytes());
...
/// 3. `[WRITE]`          member buffer PDA (`["buffer", member]`, Equinox-owned)
...
/// 8. `[]`               the system program
/// 9. `[]`               Equinox's own executable program account
///
...
pub fn delegate_cluster_member(
    ...
    let market_key = *accounts[0].address();
    {
        ...
        if header.delegation_status() != DelegationStatus::Delegated as u8 {
            return Err(custom(EquinoxError::MagicBlockNotDelegated));
        }
...
fn commit_market_inner(
    ...
    if sequence == 0 {
        return Err(custom(EquinoxError::MagicBlockSequenceReplay));
    }
    if !no_duplicate_addresses(&[
        accounts[0].address(),
        accounts[2].address(),
    ...
    if accounts.len() - 5 > MAX_COMMITTED_ACCOUNTS - 1 {
        return Err(custom(EquinoxError::MagicBlockClusterTooLarge));
    }
...
fn commit_v3_bundle(
    ...
    v3::validate_execution_bundle(program_id, &bundle, true)
        .map_err(|_| custom(EquinoxError::MagicBlockInvalidAccount))?;
    let authority = accounts[1].address().to_bytes();
    {
...
fn commit_v3_member(
    ...
    let child_index =
        v3_child_index(&bytes).map_err(|_| custom(EquinoxError::MagicBlockInvalidAccount))?;
...
/// Equinox's choice, it is the delegation program's fixed wire format):
...
pub fn external_undelegate(
    ...
    {
        return Err(custom(EquinoxError::MagicBlockInvalidCallback));
    }
    let kind = parse_delegated_seeds(&data[8..])
        .ok_or(custom(EquinoxError::MagicBlockInvalidCallback))?;
    ...
    match kind {
        DelegatedAccountKind::Market => {
            ...
            let instrument_bytes: [u8; 32] = payload[payload.len() - 32..]
                .try_into()
                .map_err(|_| custom(EquinoxError::MagicBlockInvalidCallback))?;
