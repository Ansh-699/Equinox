...
use crate::{
    ...
    error::EquinoxError,
    instruction::{PlaceOrderData, EquinoxInstruction},
    risk,
    scratch::{
...
pub(crate) fn custom(error: EquinoxError) -> ProgramError {
    error.into()
}

pub(crate) fn read_header(data: &[u8]) -> Result<MarketStateHeader, ProgramError> {
    if data.len() < MARKET_HEADER_SIZE {
        return Err(custom(EquinoxError::InvalidMarketLayout));
    }
...
pub(crate) fn initialized_header(data: &[u8]) -> Result<MarketStateHeader, ProgramError> {
    ...
    header
        .validate(data.len())
        .map_err(|_| custom(EquinoxError::InvalidMarketLayout))?;
    if header.initialized == 0 {
        return Err(custom(EquinoxError::MarketNotInitialized));
    }
    Ok(header)
}
...
fn authorize_trading_actor(
    ...
    if signer_key == seat.trader {
        ...
        if action_nonce != 0 {
            return Err(custom(EquinoxError::InvalidInstruction));
        }
        return Ok(None);
    }
    signer(&accounts[1])?;
    ...
    {
        return Err(custom(EquinoxError::InvalidTradingSession));
    }
    ...
    if action_nonce != trading_session.next_expected_nonce {
        return Err(custom(EquinoxError::SessionNonceReplay));
    }
    if trading_session.next_expected_nonce == u64::MAX {
        return Err(custom(EquinoxError::ArithmeticOverflow));
    }
    ...
    {
        return Err(custom(EquinoxError::RiskViolation));
    }
...
pub(crate) fn consume_session_action(
    ...
    let increment = u64::try_from(notional).map_err(|_| custom(EquinoxError::RiskViolation))?;
    let next_consumed = trading_session
        .consumed_cumulative_notional
        .checked_add(increment)
        .ok_or(custom(EquinoxError::RiskViolation))?;
    ...
    trading_session.next_expected_nonce = trading_session
        .next_expected_nonce
        .checked_add(1)
        .ok_or(custom(EquinoxError::ArithmeticOverflow))?;
...
pub(crate) fn seat_at(data: &[u8], index: usize) -> Result<TraderSeat, ProgramError> {
    if index >= MAX_TRADER_SEATS {
        return Err(custom(EquinoxError::InvalidSeat));
    }
...
pub fn dispatch(
    ...
    instruction: EquinoxInstruction,
    instruction_data: &[u8],
) -> ProgramResult {
    match instruction {
        EquinoxInstruction::InitializeMarket => initialize_market(program_id, accounts),
        EquinoxInstruction::CreateTraderSeat { seat_index } => {
            create_seat(program_id, accounts, seat_index as usize)
        }
        EquinoxInstruction::CloseTraderSeat { seat_index } => {
            close_seat(program_id, accounts, seat_index as usize)
        }
        EquinoxInstruction::PlaceOrder(order) => place_order(program_id, accounts, order),
        EquinoxInstruction::CancelOrder {
            seat_index,
            order_key,
            action_nonce,
        } => cancel_order(
        ...
        funding @ EquinoxInstruction::UpdateFunding { .. } => {
        ...
        EquinoxInstruction::Liquidate {
            seat_index,
            max_quantity,
        } => liquidate(program_id, accounts, seat_index as usize, max_quantity),
        EquinoxInstruction::InitializeSettlementScratch { seat_index } => {
            initialize_settlement_scratch(program_id, accounts, seat_index)
        }
        EquinoxInstruction::InitializeVault => initialize_vault(program_id, accounts),
        EquinoxInstruction::DepositCollateral { seat_index, amount } => {
            deposit_collateral(program_id, accounts, seat_index as usize, amount)
        }
        EquinoxInstruction::WithdrawCollateral { seat_index, amount } => {
            withdraw_collateral(program_id, accounts, seat_index as usize, amount)
        }
        EquinoxInstruction::ConsumeOracleUpdate => {
            if accounts.len() == 11 {
        ...
        EquinoxInstruction::DelegateMarket { validator } => crate::magicblock::delegate_market(
            program_id,
            accounts,
            Address::new_from_array(validator),
        ),
        EquinoxInstruction::DelegateClusterMember { validator } => {
        ...
        EquinoxInstruction::RevokeTradingSession { seat_index } => {
            revoke_trading_session(program_id, accounts, seat_index)
        }
        EquinoxInstruction::UpdateTradingSessionLimits {
            seat_index,
            expires_at,
            actions,
            max_order_notional,
        ...
        EquinoxInstruction::SuspendStockInstrument { instrument_id } => {
            crate::registry::suspend_instrument(program_id, accounts, instrument_id)
        }
        EquinoxInstruction::UpdateMarketRisk {
            initial_margin_bps,
        ...
        EquinoxInstruction::UpdateV3Risk {
            initial_margin_bps,
            maintenance_margin_bps,
            liquidation_fee_bps,
            maker_fee_bps,
        ...
        EquinoxInstruction::ReconcileVaultV3 => {
            crate::v3::reconcile_vault_v3(program_id, accounts)
        }
        EquinoxInstruction::CreateV3VaultAccount => {
            crate::v3::create_v3_vault_account(program_id, accounts)
        }
        EquinoxInstruction::TransitionMarket { mode, action } => {
            transition_market(program_id, accounts, mode, action)
        }
        EquinoxInstruction::TransferToInsuranceFund { amount } => {
            transfer_to_insurance_fund(program_id, accounts, amount)
        }
        EquinoxInstruction::WithdrawProtocolFees { amount } => {
            withdraw_protocol_fees(program_id, accounts, amount)
        }
        EquinoxInstruction::WithdrawInsuranceFunds { amount } => {
            withdraw_insurance_funds(program_id, accounts, amount)
        }
        EquinoxInstruction::RecordBadDebt { seat_index, amount } => {
            record_bad_debt(program_id, accounts, seat_index, amount)
        }
        EquinoxInstruction::ResolveBadDebt { amount } => {
            resolve_bad_debt(program_id, accounts, amount)
        }
        EquinoxInstruction::ReconcileVault => reconcile_vault(program_id, accounts),
        EquinoxInstruction::UpdateExchangeConfig {
            field_mask,
            pause_authority,
            emergency_authority,
            keeper_authority,
...
fn scratch_data<'a>(
    ...
    if account.data_len() != SETTLEMENT_SCRATCH_LEN {
        return Err(custom(EquinoxError::InvalidSettlementScratch));
    }
...
pub(crate) fn initialize_market_account(
    ...
    if existing.discriminator == MARKET_DISCRIMINATOR && existing.initialized != 0 {
        return Err(custom(EquinoxError::MarketAlreadyInitialized));
    }
...
fn initialize_vault(program_id: &Address, accounts: &mut [AccountView]) -> ProgramResult {
    ...
    let decimals = {
        let mint = pinocchio_token::state::Mint::from_account_view(&accounts[2])
            .map_err(|_| custom(EquinoxError::InvalidInstruction))?;
...
fn withdraw_collateral(
    ...
    if !header.l1_withdrawals_allowed() {
        return Err(custom(EquinoxError::MagicBlockUndelegationInProgress));
    }
    ...
    if header.withdrawals_blocked_by_reconciliation() {
        return Err(custom(EquinoxError::CustodyViolation));
    }
    ...
    .map_err(|_| custom(EquinoxError::RiskViolation))?;
    {
    ...
    let vault_after = {
        ...
        vault_state
            .amount()
            .checked_sub(amount)
            .ok_or(custom(EquinoxError::CustodyViolation))?
    };
    let seeds = [
...
fn withdraw_ledger_balance(
    ...
    Transfer::<&AccountView>::new(&rest[1], &rest[3], &rest[2], amount)
        .invoke_signed_with_program(&signer_seeds, rest[5].address())
        .map_err(|_| custom(EquinoxError::CustodyViolation))?;
    if from_insurance {
...
fn record_bad_debt(
    ...
    if header.oracle_valid != 1 {
        return Err(custom(EquinoxError::OracleUnavailable));
    }
    ...
    let (status, surplus, kind) = if actual == expected {
        ...
        (
            ...
            u64::try_from(actual - expected)
                .map_err(|_| custom(EquinoxError::ArithmeticOverflow))?,
...
fn consume_oracle_update(
    ...
    header.mode = match verified.session {
        ...
        _ => return Err(custom(EquinoxError::OracleUnavailable)),
    };
    let sequence = next_event_sequence(&mut header)?;
    write_header(data, &header)?;
    crate::events::emit_event(
        ...
        || i32::from(verified.exponent)
            != i32::from_le_bytes(
                core_data[crate::v3::V3_CORE_ORACLE_EXPONENT_OFFSET
                    ...
                    .try_into()
                    .map_err(|_| custom(EquinoxError::OracleUnavailable))?,
            )
        || verified.price <= 0
        || verified.confidence < 0
        || verified.confidence as u64 > verified.price.unsigned_abs() / 5
...
fn create_seat(program_id: &Address, accounts: &mut [AccountView], index: usize) -> ProgramResult {
    ...
    while i < MAX_TRADER_SEATS {
        ...
        if !seat.is_empty() && seat.trader == trader {
            return Err(custom(EquinoxError::SeatOccupied));
        }
        i += 1;
    }
...
fn close_seat(program_id: &Address, accounts: &mut [AccountView], index: usize) -> ProgramResult {
    ...
    if !seat.can_close() {
        return Err(custom(EquinoxError::SeatNotEmpty));
    }
...
fn place_order_core(
    ...
    let self_trade_behavior = crate::book::SelfTradeBehavior::from_u8((order.flags >> 3) & 0b11)
        .ok_or(custom(EquinoxError::InvalidInstruction))?;
    ...
    if scratch.plan().self_trade_aborted {
        ...
        scratch.abort_to(&scratch_before);
        return Err(custom(EquinoxError::SelfTradeAborted));
    }
    ...
    if total_fee > 0 {
        let fee_u64 =
            u64::try_from(total_fee).map_err(|_| custom(EquinoxError::ArithmeticOverflow))?;
...
fn cancel_order_core(data: &mut [u8], seat_index: usize, order_key: u128) -> ProgramResult {
    ...
    let leaf = removed.map_err(|_| custom(EquinoxError::InvalidInstruction))?;
    let mut seat = seat_at(data, seat_index)?;
...
fn update_funding(
    ...
    let EquinoxInstruction::UpdateFunding {
        accumulator,
        timestamp,
    } = instruction
    else {
...
fn risk_error(error: risk::RiskError) -> ProgramError {
    match error {
        risk::RiskError::Overflow => custom(EquinoxError::ArithmeticOverflow),
        _ => custom(EquinoxError::RiskViolation),
    }
}

fn book_error(_error: crate::book::BookError) -> ProgramError {
    custom(EquinoxError::InvalidInstruction)
}
