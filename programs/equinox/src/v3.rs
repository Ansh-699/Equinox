...
fn fresh_v3_execution_time_with_snapshot(
    ...
    if let Some(index) = snapshot_index {
        ...
        let feed = u32::from_le_bytes(
            core[V3_CORE_ORACLE_FEED_ID_OFFSET..V3_CORE_ORACLE_FEED_ID_OFFSET + 4]
                .try_into()
                .map_err(|_| bundle_error())?,
        );
        let channel = core[V3_CORE_ORACLE_CHANNEL_OFFSET];
        let exponent = i32::from_le_bytes(
            core[V3_CORE_ORACLE_EXPONENT_OFFSET..V3_CORE_ORACLE_EXPONENT_OFFSET + 4]
                .try_into()
                .map_err(|_| bundle_error())?,
        );
...
fn optional_snapshot_index(accounts: &[AccountView]) -> Option<usize> {
    accounts
        .iter()
        .enumerate()
        .skip(V3_SESSION_ACCOUNT_INDEX)
        .find_map(|(index, account)| {
            if account.data_len() == crate::oracle_snapshot::ORACLE_SNAPSHOT_SIZE
                && unsafe { account.borrow_unchecked() }[0..8]
                    == crate::oracle_snapshot::ORACLE_SNAPSHOT_DISCRIMINATOR
            {
                Some(index)
            } else {
                None
...
/// Returns the authenticated mark after `fresh_v3_execution_time_with_snapshot`
/// has succeeded. When a snapshot is supplied it is the sole price source;
/// the delegated core's cached oracle bytes are only the legacy fallback when
/// no snapshot account is present.
fn oracle_price_after_freshness(
    accounts: &[AccountView],
    snapshot_index: Option<usize>,
    ...
    let snapshot = snapshot_index
        ...
        .transpose()?
        .map(|account| unsafe { account.borrow_unchecked() });
    let price = oracle_price_from_bytes(&core, snapshot.as_deref())?;
    if price <= 0 {
        return Err(EquinoxError::OracleUnavailable.into());
    }
    Ok(price)
...
fn oracle_price_from_bytes(core: &[u8], snapshot: Option<&[u8]>) -> Result<i64, ProgramError> {
    let bytes = snapshot.unwrap_or(core);
    let offset = if snapshot.is_some() {
        crate::oracle_snapshot::OFFSET_PRICE
    } else {
        V3_CORE_ORACLE_PRICE_OFFSET
    };
    let end = offset
        .checked_add(8)
    ...
    Ok(i64::from_le_bytes(
        bytes[offset..end].try_into().map_err(|_| bundle_error())?,
    ))
}
...
impl<'a> PagedBookV3<'a> {
    ...
    fn allocate(&mut self) -> Result<u32, ProgramError> {
        ...
        if bump as usize >= V3_BOOK_SLOTS_PER_SIDE {
            return Err(EquinoxError::V3BookFull.into());
        }
    ...
    fn plan_crossing_trees(
        ...
        while selected_count < crate::book::MAX_FILLS_PER_INSTRUCTION && plan.taker_remaining > 0 {
            ...
            if owner == taker_owner {
                match self_trade_behavior {
                    SelfTradeBehavior::AbortTransaction => {
                        return Err(EquinoxError::SelfTradeAborted.into())
                    }
                    ...
                    SelfTradeBehavior::CancelProvide => {
                        ...
                        let reserve_price = match stored_v3_reserve_price(&leaf) {
                            Some(price) => u64::try_from(price)
                                .map_err(|_| EquinoxError::ArithmeticOverflow)?,
                            None => maker_price,
                        };
            ...
            let reserve_price = match stored_v3_reserve_price(&leaf) {
                Some(price) => {
                    u64::try_from(price).map_err(|_| EquinoxError::ArithmeticOverflow)?
                }
                None => maker_price,
            };
...
pub fn update_funding_v3(
    ...
    let now = fresh_v3_execution_time_with_snapshot(program_id, accounts, snapshot_index)?;
    let oracle = oracle_price_after_freshness(accounts, snapshot_index)?;
...
fn bundle_error() -> ProgramError {
    EquinoxError::InvalidInstruction.into()
}

fn v3_risk_error(error: crate::risk::RiskError) -> ProgramError {
    match error {
        crate::risk::RiskError::Overflow => EquinoxError::ArithmeticOverflow.into(),
        crate::risk::RiskError::InvalidPrice => EquinoxError::InvalidInstruction.into(),
        crate::risk::RiskError::NegativeCollateral
        | crate::risk::RiskError::PositionLimit
        | crate::risk::RiskError::OpenInterestLimit
        | crate::risk::RiskError::Margin => EquinoxError::RiskViolation.into(),
    }
}
...
fn validate_execution_bundle_inner(
    ...
    {
        return Err(EquinoxError::MagicBlockUndelegationInProgress.into());
    }
...
pub fn validate_v3_session_actor(
    ...
    if action_nonce != session_state.next_expected_nonce {
        return Err(EquinoxError::SessionNonceReplay.into());
    }
    if session_state.next_expected_nonce == u64::MAX {
        return Err(EquinoxError::ArithmeticOverflow.into());
    }
    if notional < 0
    ...
    for shard in 0..V3_SEAT_SHARDS {
        ...
        for slot in 0..V3_SEATS_PER_SHARD {
            ...
            if !seat.is_empty() && seat.trader == trader {
                return Err(EquinoxError::SeatOccupied.into());
            }
        }
    }
    let shard = index / V3_SEATS_PER_SHARD;
...
pub fn authorize_trading_session_v3(
    ...
    if accounts.len() != V3_EXECUTION_BUNDLE_LEN + 4
        && accounts.len() != V3_EXECUTION_BUNDLE_LEN + 5
    {
        pinocchio_log::log!(
            256,
    ...
    validate_execution_bundle(program_id, &accounts[..V3_EXECUTION_BUNDLE_LEN], true)?;
    let snapshot_index = if accounts.len() == V3_EXECUTION_BUNDLE_LEN + 5 {
        Some(V3_EXECUTION_BUNDLE_LEN + 4)
    } else {
        None
    };
    ...
    if snapshot_index.is_none() && core_snapshot[V3_CORE_ORACLE_VALID_OFFSET] != 1
        || expires_at <= now
        || actions == 0
    ...
    {
        pinocchio_log::log!(256, "auth_session:FAIL oracle_valid={} snapshot={} expires_at={} now={} actions={} max_order_notional={} max_cum={} max_exposure={} max_open_orders={}", core_snapshot[V3_CORE_ORACLE_VALID_OFFSET], snapshot_index.is_some(), expires_at, now, actions, max_order_notional, max_cumulative_notional, maximum_exposure, maximum_open_orders);
        return Err(EquinoxError::InvalidTradingSession.into());
    }
...
pub fn close_trader_seat(
    ...
    let trader = accounts[9].address().to_bytes();
    {
        ...
        if !seat.can_close() {
            return Err(EquinoxError::SeatNotEmpty.into());
        }
...
fn validate_v3_trade_accounts(
    ...
    if accounts.len() > V3_SESSION_ACCOUNT_INDEX + 2 {
        fresh_v3_execution_time_with_snapshot(
            program_id,
            accounts,
            optional_snapshot_index(accounts),
        )?;
    }
    validate_execution_bundle(program_id, &accounts[..V3_EXECUTION_BUNDLE_LEN], true)?;
    let core = &accounts[0];
...
fn settle_v3_fill(
    ...
    maker.reserved_margin = maker
        .reserved_margin
        .checked_sub(margin)
        .ok_or(EquinoxError::RiskViolation)?;
    if fill.maker_remaining == 0 {
...
fn v3_order_reserve(
    ...
    let price = match tree {
        ...
        TreeKind::OraclePegged => stored_v3_reserve_price(leaf).unwrap_or(
            oracle
                .ok_or(EquinoxError::OracleUnavailable)?
    ...
    for shard in 0..V3_SEAT_SHARDS {
        ...
        for slot in 0..V3_SEATS_PER_SHARD {
            ...
            for fill in plan.fills[..plan.fill_count as usize].iter() {
                ...
                if usize::try_from(fill.maker_owner).map_err(|_| EquinoxError::InvalidSeat)?
                    == index
                {
                    position = position
...
pub fn liquidate_v3(
    ...
    if (accounts.len() != V3_EXECUTION_BUNDLE_LEN + 1
        && accounts.len() != V3_EXECUTION_BUNDLE_LEN + 2)
        || max_quantity == 0
    {
        return Err(ProgramError::NotEnoughAccountKeys);
    }
    ...
    let (mark, funding, config) = {
        ...
        let mark = oracle_price_after_freshness(accounts, snapshot_index)?;
        if mark <= 0 {
            return Err(EquinoxError::OracleUnavailable.into());
        }
        (
...
fn place_order_v3_with_action(
    ...
    let side = match order.side {
        0 => Side::Bid,
        1 => Side::Ask,
        _ => return Err(EquinoxError::InvalidInstruction.into()),
    };
    ...
    let oracle = Some(oracle_price_after_freshness(accounts, snapshot_index)?);
    ...
    let self_trade_behavior = SelfTradeBehavior::from_u8((order.flags >> 3) & 0b11)
        .ok_or(EquinoxError::InvalidInstruction)?;
    ...
    for cancel in plan.cancellations[..plan.cancellation_count as usize].iter() {
        let payload = crate::events::payload_order(
            ...
            cancel.key,
            cancel.side,
            i64::try_from(cancel.price).map_err(|_| EquinoxError::ArithmeticOverflow)?,
            cancel.quantity,
        );
        append_event_record(
            program_id,
    ...
    for fill in plan.fills[..plan.fill_count as usize].iter() {
        let payload = crate::events::payload_fill(
            fill.maker_owner,
            order.seat_index as u32,
            i64::try_from(fill.price).map_err(|_| EquinoxError::ArithmeticOverflow)?,
            fill.quantity,
            sequence,
        );
        append_event_record(
...
pub fn reconcile_vault_v3(program_id: &Address, accounts: &mut [AccountView]) -> ProgramResult {
    ...
    let vault = TokenAccount::from_account_view(&accounts[V3_EXECUTION_BUNDLE_LEN])
        .map_err(|_| EquinoxError::CustodyViolation)?;
    ...
    let expected_u64 = u64::try_from(expected).map_err(|_| EquinoxError::ArithmeticOverflow)?;
    let actual_u64 = u64::try_from(actual).map_err(|_| EquinoxError::ArithmeticOverflow)?;
...
pub fn deposit_collateral_v3(
    ...
    if (accounts.len() != 11 && accounts.len() != 12) || amount == 0 {
        return Err(ProgramError::NotEnoughAccountKeys);
    }
    ...
    let core_key = *core.address();
    let core_bytes = unsafe { core.borrow_unchecked() };
    let snapshot_index = (accounts.len() == 12).then_some(11);
    fresh_v3_execution_time_with_snapshot(program_id, accounts, snapshot_index)?;
...
pub fn withdraw_collateral_v3(
    ...
    if (accounts.len() != V3_EXECUTION_BUNDLE_LEN + 6
        && accounts.len() != V3_EXECUTION_BUNDLE_LEN + 7)
        || amount == 0
    {
        return Err(ProgramError::NotEnoughAccountKeys);
    }
    ...
    let snapshot_index =
        (accounts.len() == V3_EXECUTION_BUNDLE_LEN + 7).then_some(V3_EXECUTION_BUNDLE_LEN + 6);
    if snapshot_index.is_none() && core_bytes[V3_CORE_ORACLE_VALID_OFFSET] == 0 {
        return Err(EquinoxError::RiskViolation.into());
    }
    fresh_v3_execution_time_with_snapshot(program_id, accounts, snapshot_index)?;
    let mark_price = oracle_price_after_freshness(accounts, snapshot_index)?;
...
fn cancel_order_v3_with_action(
    ...
    let (side, tree, leaf) = removed.ok_or(EquinoxError::InvalidInstruction)?;
...
fn preflight_replace_order_v3(
    ...
    let (old_tree, old_leaf) = old_leaf.ok_or(EquinoxError::InvalidInstruction)?;
