...
/// status, a config sequence). Equinox has never been deployed, so
...
///    `system::allocate(10,240)` + `system::assign(Equinox)` via CPIs
...
pub fn create_market_account(program_id: &Address, accounts: &mut [AccountView]) -> ProgramResult {
    ...
    // Growth phase: the market PDA must be Equinox-owned now.
...
fn custom(error: EquinoxError) -> ProgramError {
    error.into()
}
