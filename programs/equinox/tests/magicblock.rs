...
//!
//! 1. The exact bytes Equinox would send are correct -- checked by
...
fn schedule_commit_data_matches_real_bincode_instruction() {
    ...
    let complete_v3_bundle =
        (2..(2 + equinox::v3::V3_EXECUTION_BUNDLE_LEN as u8)).collect::<Vec<_>>();
    for indices in [vec![2u8], vec![2u8, 3, 4], complete_v3_bundle] {
        ...
        let len =
            equinox::magicblock::encode_schedule_intent_bundle_data(&indices, false, &mut ours)
                .map_err(|_| ())
                .unwrap();
...
fn schedule_commit_and_undelegate_data_matches_real_bincode_instruction() {
    ...
    for indices in [vec![2u8], vec![2u8, 3, 4]] {
        ...
        let mut ours = [0u8; SCHEDULE_DATA_MAX_LEN];
        let len =
            equinox::magicblock::encode_schedule_intent_bundle_data(&indices, true, &mut ours)
                .map_err(|_| ())
                .unwrap();
        assert_eq!(&ours[..len], expected.as_slice());
    }
...
fn undelegate_seeds_encoding_round_trips_through_the_same_encoder_used_for_delegate() {
    use equinox::magicblock::parse_delegated_seeds;
    ...
    assert!(matches!(
        ...
        Some(equinox::magicblock::DelegatedAccountKind::Market)
    ));
}
...
fn err_code(result: pinocchio::ProgramResult) -> u32 {
    match result.unwrap_err() {
        ...
        other => panic!("expected a custom Equinox error, got {other:?}"),
    }
}
...
fn delegate_market_rejects_zero_validator() {
    ...
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockInvalidAccount as u32
    );
}
...
fn delegate_market_rejects_non_empty_settlement_scratch() {
    ...
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockScratchNotEmpty as u32
    );
}
...
fn delegate_market_rejects_already_delegated_market() {
    ...
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockAlreadyDelegated as u32
    );
}
...
fn commit_market_rejects_when_not_delegated() {
    ...
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockNotDelegated as u32
    );
}
...
fn commit_market_rejects_sequence_replay() {
    ...
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockSequenceReplay as u32
    );
...
fn commit_and_undelegate_rejects_when_already_pending() {
    ...
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockUndelegationInProgress as u32
    );
}
...
fn external_undelegate_rejects_wrong_discriminator() {
    ...
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockInvalidCallback as u32
    );
}
...
#[test]
fn external_undelegate_rejects_a_market_still_owned_by_equinox() {
    let instrument_key = Address::new_from_array([1; 32]);
    let market_key = derive_perp_market(&ID, &instrument_key);
    // Still owned by Equinox: this account was never actually handed to
...
fn cluster_member_seed_encodings_match_real_borsh_delegate_args() {
    ...
    let ours = equinox::magicblock::encode_scratch_delegate_seeds(&market_key, seat);
    assert_eq!(
        ours.len(),
        equinox::magicblock::SCRATCH_SEEDS_PAYLOAD_LEN
    );
    ...
    let ours = equinox::magicblock::encode_session_delegate_seeds(
        &owner,
        &market_key,
        seat,
        &session_signer,
    ...
    assert_eq!(
        ours.len(),
        equinox::magicblock::SESSION_SEEDS_PAYLOAD_LEN
    );
    let mut expected = Vec::new();
    dlp_api::args::DelegateArgs {
        commit_frequency_ms: 0,
    ...
    let mut out = [0u8; equinox::magicblock::DELEGATE_INSTRUCTION_DATA_MAX_LEN];
    let len = equinox::magicblock::encode_delegate_instruction_data_from_seeds(
    ...
    let ours = equinox::magicblock::encode_v3_book_page_delegate_seeds(&market_key, 1, 3);
    let mut expected = Vec::new();
    dlp_api::args::DelegateArgs {
        commit_frequency_ms: 0,
        seeds: vec![
...
fn parse_delegated_seeds_round_trips_every_kind_and_rejects_foreign_shapes() {
    use equinox::magicblock::{parse_delegated_seeds, DelegatedAccountKind};
    ...
    let scratch_seeds = equinox::magicblock::encode_scratch_delegate_seeds(&market_key, seat);
    assert_eq!(
    ...
    let session_seeds = equinox::magicblock::encode_session_delegate_seeds(
        &owner,
        &market_key,
        seat,
        &session_signer,
    ...
    let core_seeds = equinox::magicblock::encode_v3_core_delegate_seeds(&instrument);
    assert_eq!(
        parse_delegated_seeds(&core_seeds),
        Some(DelegatedAccountKind::V3Core { instrument })
    );
    let page_seeds = equinox::magicblock::encode_v3_book_page_delegate_seeds(&market_key, 1, 3);
    assert_eq!(
    ...
    let seat_shard_seeds =
        equinox::magicblock::encode_v3_seat_shard_delegate_seeds(&market_key, 2);
    assert_eq!(
    ...
    let event_shard_seeds =
        equinox::magicblock::encode_v3_event_shard_delegate_seeds(&market_key, 3);
    assert_eq!(
    ...
    assert_eq!(
        parse_delegated_seeds(&[0u8; equinox::magicblock::MARKET_SEEDS_PAYLOAD_LEN]),
        None
    );
...
fn session_account(
    ...
    let address =
        equinox::session::derive_trading_session(&owner, &market, seat, &session_signer, &ID);
    let mut acct = account(address, owner_of_account, 256, false, true);
    let mut session = equinox::session::TradingSession::empty();
    ...
    bytes.fill(0);
    equinox::session::write_session(bytes, &session).unwrap();
    acct
}
...
fn delegate_cluster_member_rejects_a_market_that_is_not_delegated() {
    ...
    let result = equinox::magicblock::delegate_cluster_member(&ID, &mut accounts, validator);
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockNotDelegated as u32
    );
...
fn delegate_cluster_member_rejects_a_validator_different_from_the_markets() {
    ...
    let result =
        equinox::magicblock::delegate_cluster_member(&ID, &mut accounts, other_validator);
    assert_eq!(
        err_code(result),
        EquinoxError::MagicBlockInvalidAccount as u32
    );
...
fn external_undelegate_rejects_a_scratch_callback_for_a_foreign_account() {
    ...
    let other = {
        let address = equinox::session::derive_trading_session(
            &owner,
            &market_key,
            0,
            &session_signer,
...
fn restore_validation_accepts_committed_scratch_and_session_bytes() {
    ...
    use equinox::magicblock::{validate_restored_scratch, validate_restored_session};
    ...
    equinox::session::write_session(&mut session_bytes, &session).unwrap();
    ...
    equinox::session::write_session(&mut tampered_bytes, &session).unwrap();
    assert!(validate_restored_session(
        &tampered_bytes,
        &owner,
        &market_key,
