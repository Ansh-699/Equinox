...
//! Real-SVM runtime tests for the Equinox collateral **deposit** path.
//!
...
//! `equinox.so` inside LiteSVM, where LiteSVM has already installed the
...
/// The canonical deposit account list (`clients/equinox` order).
...
fn deposit_rolls_back_when_the_tokenkeg_cpi_fails() {
    ...
    // Freeze the source account. Every Equinox pre-check still passes
