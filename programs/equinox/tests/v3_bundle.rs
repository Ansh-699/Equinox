...
fn bundle() -> Vec<TestAccount> {
    ...
    equinox::v3::initialize_v3_risk_config(unsafe { core.view.borrow_unchecked_mut() })
        .unwrap();
    unsafe {
        ...
        data[189..197]
            .copy_from_slice(&(equinox::handlers::OFF_CHAIN_TEST_NOW as u64).to_le_bytes());
    }
...
fn er_readonly_snapshot_boundary_accepts_fresh_snapshot_without_pyth_accounts() {
    ...
    let now = equinox::handlers::OFF_CHAIN_TEST_NOW as u64;
    unsafe {
        let core = accounts[0].view.borrow_unchecked_mut();
        core[equinox::v3::V3_CORE_ORACLE_FEED_ID_OFFSET
            ..equinox::v3::V3_CORE_ORACLE_FEED_ID_OFFSET + 4]
            .copy_from_slice(&1435u32.to_le_bytes());
        core[equinox::v3::V3_CORE_ORACLE_CHANNEL_OFFSET] = 2;
        core[equinox::v3::V3_CORE_ORACLE_EXPONENT_OFFSET
    ...
    equinox::oracle_snapshot::write_verified(
...
fn v3_session_authorization_uses_precreated_account_without_owner_debit_or_cpi() {
    ...
    authorize_trading_session_v3(
        ...
        &mut auth,
        0,
        equinox::handlers::OFF_CHAIN_TEST_NOW as u64 + 600,
...
fn v3_funding_recomputes_mark_from_pages_and_emits_event() {
    ...
    update_funding_v3(
        &ID,
        &mut call,
        EquinoxInstruction::UpdateFunding {
            accumulator: 1,
            timestamp: 20,
        },
    )
...
fn v3_keeper_set_by_authority_may_run_funding_but_a_stranger_may_not() {
    ...
    let funding = |signer: &TestAccount, accounts: &[TestAccount], accumulator| {
        ...
        update_funding_v3(&ID, &mut call, EquinoxInstruction::UpdateFunding { accumulator, timestamp: 20 })
    };
...
fn v3_funding_uses_configured_mark_deviation_clamp() {
    ...
    let authority = account(Address::new_from_array([78; 32]), 0, true);
    {
        ...
        core[371] = equinox::v3::V3_RISK_CONFIG_VERSION;
    }
    let mut page_views = views(&accounts);
    let mut bid_book = PagedBookV3::new(&mut page_views[1..1 + V3_BOOK_PAGES_PER_SIDE]).unwrap();
    let mut bid = leaf(1, 0);
...
fn v3_owner_place_reserves_collateral_and_writes_paged_book_and_event() {
    ...
    assert!(equinox::v3::cancel_order_v3(
        &ID,
        &mut failed_cancel,
        0,
    ...
    equinox::v3::cancel_order_v3(&ID, &mut cancel_accounts, 0, order_key, 0).unwrap();
    let seat = unsafe { accounts[V3_SEAT_START].view.borrow_unchecked() };
...
fn v3_reconcile_rejects_delegated_core_without_mutation() {
    ...
    assert!(equinox::v3::reconcile_vault_v3(&ID, &mut call).is_err());
    assert_eq!(
...
fn v3_session_cancel_all_consumes_one_nonce_and_rejects_replay() {
    ...
    place_order_v3(
        ...
        PlaceOrderData {
            ...
            quantity: 2,
            price_or_offset: 5,
            expires_at: equinox::handlers::OFF_CHAIN_TEST_NOW as u64 + 100,
            peg_limit: 0,
            client_order_id: 54,
            action_nonce: 0,
        },
    ...
    state.expires_at = equinox::handlers::OFF_CHAIN_TEST_NOW as u64 + 100;
...
fn v3_session_replace_consumes_one_nonce_and_requires_replace_permission() {
    ...
    assert!(equinox::v3::replace_order_v3(
        &ID,
...
fn v3_paged_book_preserves_global_handles_across_page_boundaries() {
    ...
    let plan = book
        .plan_crossing(
            ...
            Side::Bid,
            2_000,
            equinox::book::SelfTradeBehavior::AbortTransaction,
            2_000,
            1,
            None,
            0,
...
fn v3_self_trade_policies_apply_on_paged_books() {
    ...
    assert_eq!(
        ...
        .unwrap_err(),
        equinox::error::EquinoxError::SelfTradeAborted.into()
    );
    let decrement = book
        .plan_crossing(
            TreeKind::Fixed,
            Side::Bid,
            7,
            equinox::book::SelfTradeBehavior::DecrementTake,
            20,
            1,
            None,
            0,
    ...
    let cancel = book
        .plan_crossing(
            ...
            Side::Bid,
            7,
            equinox::book::SelfTradeBehavior::CancelProvide,
            20,
            1,
            None,
            0,
...
fn withdrawal_price_accounts(snapshot_core: Address, status: u8, publish: u64) -> (Vec<TestAccount>, TestAccount) {
    use equinox::oracle_snapshot as snap;
    let mut accounts = bundle();
    unsafe {
        let core = accounts[0].view.borrow_unchecked_mut();
...
fn withdrawals_never_wait_for_a_live_price() {
    use equinox::oracle_snapshot::{STATUS_CLOSED, STATUS_OPEN};
    use equinox::v3::withdrawal_mark_price;
    let now = equinox::handlers::OFF_CHAIN_TEST_NOW as u64;
...
fn price_reporter_posts_bounded_prices_only_for_its_own_market() {
    use equinox::handlers::{dispatch, OFF_CHAIN_TEST_NOW};
    use equinox::instruction::EquinoxInstruction as Ix;
    use equinox::oracle_snapshot as snap;
    ...
    let snapshot_key = equinox::v3::derive_oracle_snapshot_v3(&ID, &core_key);
