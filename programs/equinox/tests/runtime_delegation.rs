...
//! the target market, executed against the actual deployed `equinox.so`
//! inside LiteSVM (no host-side mocking of the validation logic).
//!
//! Contract under test (`programs/equinox/src/magicblock.rs`):
//!
...
fn commit_rejects_a_non_member_trailing_account() {
    ...
    let instruction = Instruction {
        ...
        accounts: vec![
            ...
            AccountMeta {
                pubkey: equinox::magicblock::MAGIC_CONTEXT_ID,
                is_signer: false,
                is_writable: true,
            },
            AccountMeta {
                pubkey: equinox::magicblock::MAGIC_PROGRAM_ID,
                is_signer: false,
                is_writable: false,
            },
            AccountMeta {
...
fn commit_rejects_a_replayed_sequence() {
    ...
    // (Equinox) and delegation fields — simulate that by re-installing
