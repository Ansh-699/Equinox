...
fn registry_update_and_suspend_preserve_feed_configuration() {
    use equinox::registry::{derive_instrument, EXCHANGE_SIZE, INSTRUMENT_SIZE};
    let owner = account(
...
fn trading_session_is_bound_to_the_owners_actual_seat_and_can_be_revoked() {
    use equinox::session::{derive_trading_session, read_session, TRADING_SESSION_SIZE};
    ...
    // Not yet owned by Equinox: `AuthorizeTradingSession` creates it via
...
/// `clients/equinox/src/index.ts::decodeMarketState` reads
...
fn custody_ledger_field_offsets_match_the_typescript_decoder() {
    ...
    assert_eq!(
        RESERVED_UPGRADE_OFFSET + equinox::state::RESERVED_PROTOCOL_FEE_BALANCE,
        449
    );
    assert_eq!(
        RESERVED_UPGRADE_OFFSET + equinox::state::RESERVED_INSURANCE_FUND_BALANCE,
        457
    );
    assert_eq!(
        RESERVED_UPGRADE_OFFSET + equinox::state::RESERVED_RECOGNIZED_BAD_DEBT,
        465
    );
    assert_eq!(
        RESERVED_UPGRADE_OFFSET + equinox::state::RESERVED_RECONCILIATION_STATUS,
        473
    );
    assert_eq!(
        RESERVED_UPGRADE_OFFSET + equinox::state::RESERVED_VAULT_SURPLUS,
        474
    );
}

/// Every field the Worker's authoritative market-state reader
/// (`clients/equinox/src/index.ts::decodeMarketState`) reads beyond the
...
fn prepare_crossing_plan(f: &mut Fixture) -> (OrderInput, MarketStateHeader, TraderSeat) {
        ...
        .unwrap();
    {
        ...
        let bids =
            unsafe { &*(data.as_ptr().add(equinox::state::BID_ARENA_OFFSET) as *const Arena) };
        let asks =
            unsafe { &*(data.as_ptr().add(equinox::state::ASK_ARENA_OFFSET) as *const Arena) };
        plan_limit_arenas_into(
            ...
            MatchLimits {
                max_fills: equinox::book::MAX_FILLS_PER_INSTRUCTION as u8,
...
fn serialized_market_can_create_seats_and_settle_crossing_orders() {
    ...
    let event_offset = equinox::state::FILL_EVENT_OFFSET
        + (fill_sequence as usize % equinox::state::FILL_EVENT_CAPACITY)
            * equinox::state::FILL_EVENT_SIZE;
    let event =
        unsafe { &*(data.as_ptr().add(event_offset) as *const equinox::state::FillEvent) };
...
fn mvp_partial_fill_preserves_remaining_leaf_and_reserve() {
    ...
    assert_eq!(
        maker_reserve,
        equinox::risk::initial_margin(equinox::risk::notional(6, 100).unwrap(), 2_000)
            .unwrap()
    );
    let arena = unsafe {
        &*(data.as_ptr().add(equinox::state::ASK_ARENA_OFFSET)
            as *const equinox::book::Arena)
    };
...
fn mvp_ioc_partial_fill_leaves_no_taker_residual() {
    ...
    let bid = unsafe {
        &*(data.as_ptr().add(equinox::state::BID_ARENA_OFFSET)
            as *const equinox::book::Arena)
    };
...
fn mvp_event_ring_wraps_from_final_slot_to_zero() {
    ...
    let last = unsafe {
        &*(data
            .as_ptr()
            .add(equinox::state::FILL_EVENT_OFFSET + 127 * equinox::state::FILL_EVENT_SIZE)
            as *const equinox::state::FillEvent)
    };
    let zero = unsafe {
        &*(data.as_ptr().add(equinox::state::FILL_EVENT_OFFSET)
            as *const equinox::state::FillEvent)
    };
...
fn liquidate_instruction_reduces_an_undercollateralized_position_and_advances_events() {
    ...
    assert_eq!(
        liquidation_state,
        equinox::state::LiquidationState::Liquidatable as u8
    );
...
fn mvp_stale_plan_rejects_changed_maker_and_market_snapshots() {
    ...
    };
    {
        ...
        let ask = unsafe {
            &mut *(data.as_mut_ptr().add(equinox::state::ASK_ARENA_OFFSET) as *mut Arena)
        };
