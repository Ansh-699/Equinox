...
use equinox::instruction::EquinoxInstruction;
use equinox::{
    error::EquinoxError,
...
fn initialize_market_requires_a_writable_market() {
    ...
    assert_eq!(
        validate_initialize_market(&ID, &accounts),
        Err(EquinoxError::MarketNotWritable.into())
    );
}
...
fn production_instruction_decoder_covers_integration_variants() {
    assert!(matches!(
        EquinoxInstruction::decode(&[9]),
        Ok(EquinoxInstruction::InitializeVault)
    ));
    assert!(matches!(
        EquinoxInstruction::decode(&[10, 0, 0, 7, 0, 0, 0, 0, 0, 0, 0]),
        Ok(EquinoxInstruction::DepositCollateral {
            seat_index: 0,
            amount: 7
        })
    ));
    assert!(EquinoxInstruction::decode(&[12]).is_err());
    let mut signed = [0u8; 107];
    signed[0] = 12;
    assert!(matches!(
        EquinoxInstruction::decode(&signed),
        Ok(EquinoxInstruction::ConsumeOracleUpdate)
    ));
    assert!(matches!(
        EquinoxInstruction::decode(&[14, 1, 0, 0, 0, 0, 0, 0, 0]),
        Ok(EquinoxInstruction::CommitMarket { sequence: 1 })
    ));
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&v3_custody),
        Ok(EquinoxInstruction::DepositCollateralV3 {
            seat_index: 7,
            amount: 9
        })
    ));
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&v3_custody),
        Ok(EquinoxInstruction::WithdrawCollateralV3 {
            seat_index: 7,
            amount: 9
        })
    ));
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&session),
        Ok(EquinoxInstruction::AuthorizeTradingSession {
            seat_index: 2,
            expires_at: 20,
            ..
        })
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&instrument),
        Ok(EquinoxInstruction::UpdateStockInstrument {
            pyth_feed_id: 123,
            oracle_channel: 1,
            price_exponent: -2,
            ..
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&[25]),
        Ok(EquinoxInstruction::TransitionMarket {
            mode: 0,
            action: equinox::instruction::MarketTransitionAction::Pause
        })
    ));
    let mut risk = [0u8; 9];
    risk[0] = 24;
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&risk),
        Ok(EquinoxInstruction::UpdateMarketRisk {
            maximum_leverage: 5,
            ..
        })
    ));
...
fn all_six_market_transition_opcodes_decode_to_distinct_actions_and_correct_modes() {
    use equinox::instruction::{MarketTransitionAction as Action, EquinoxInstruction as I};
    let cases = [
        (equinox::instruction::PAUSE_MARKET, 0u8, Action::Pause),
        (equinox::instruction::RESUME_MARKET, 1, Action::Resume),
        (
            equinox::instruction::SET_CLOSE_ONLY,
            2,
            Action::SetCloseOnly,
        ),
        (
            equinox::instruction::ENTER_CORPORATE_ACTION,
            3,
            Action::EnterCorporateAction,
        ),
        (
            equinox::instruction::RESOLVE_CORPORATE_ACTION,
            1,
            Action::ResolveCorporateAction,
        ),
        (equinox::instruction::CLOSE_MARKET, 0, Action::Close),
    ];
