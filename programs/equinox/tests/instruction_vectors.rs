use equinox::instruction::{self, EquinoxInstruction};

/// Every Equinox instruction opcode must be a unique byte. This is a
...
fn truncated_payloads_are_rejected_not_defaulted() {
    ...
    assert!(EquinoxInstruction::decode(&place_order).is_err());
    // CREATE_TRADER_SEAT expects 3 bytes; a bare opcode must reject.
    assert!(EquinoxInstruction::decode(&[instruction::CREATE_TRADER_SEAT]).is_err());
}
...
fn trailing_unsupported_bytes_are_rejected() {
    ...
    assert!(EquinoxInstruction::decode(&place_order).is_ok());
...
fn unknown_opcode_is_rejected() {
    assert!(EquinoxInstruction::decode(&[200]).is_err());
    assert!(
        EquinoxInstruction::decode(&[instruction::UNDELEGATION_CALLBACK_RESERVED]).is_err()
    );
}
...
fn v3_risk_update_vector_decodes_all_persisted_fields() {
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&data),
        Ok(EquinoxInstruction::UpdateV3Risk {
...
fn v3_reconcile_vault_opcode_decodes_without_authority_payload() {
    assert!(matches!(
        EquinoxInstruction::decode(&[instruction::RECONCILE_VAULT_V3]),
        Ok(EquinoxInstruction::ReconcileVaultV3)
    ));
}
...
fn typescript_golden_instruction_vectors_decode_in_rust() {
    assert!(matches!(
        EquinoxInstruction::decode(&[0]),
        Ok(EquinoxInstruction::InitializeMarket)
    ));
    assert!(matches!(
        EquinoxInstruction::decode(&[1, 7, 0]),
        Ok(EquinoxInstruction::CreateTraderSeat { seat_index: 7 })
    ));
    assert!(matches!(
        EquinoxInstruction::decode(&[2, 7, 0]),
        Ok(EquinoxInstruction::CloseTraderSeat { seat_index: 7 })
    ));
    assert!(matches!(
        EquinoxInstruction::decode(&[5, 7, 0, 8, 9, 0, 0, 0, 0, 0, 0, 0]),
        Ok(EquinoxInstruction::CancelAll {
            seat_index: 7,
            max_cancellations: 8,
            action_nonce: 9,
        })
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&cancel),
        Ok(EquinoxInstruction::CancelOrder {
            seat_index: 7,
    ...
    assert!(matches!(
        EquinoxInstruction::decode(&[equinox::instruction::CREATE_V3_ACCOUNT, 0, 0]),
        Ok(EquinoxInstruction::CreateV3Account { kind: 0, index: 0 })
    ));
    assert!(matches!(
        EquinoxInstruction::decode(&[equinox::instruction::CREATE_V3_ACCOUNT, 1, 7]),
        Ok(EquinoxInstruction::CreateV3Account { kind: 1, index: 7 })
    ));
    assert!(matches!(
        EquinoxInstruction::decode(&[equinox::instruction::CREATE_ORACLE_SNAPSHOT_V3]),
        Ok(EquinoxInstruction::CreateOracleSnapshotV3)
    ));
    ...
    assert!(
        EquinoxInstruction::decode(&[equinox::instruction::CREATE_V3_ACCOUNT, 1, 18,])
            .is_err()
    );
    assert!(
        EquinoxInstruction::decode(&[equinox::instruction::CREATE_V3_ACCOUNT, 4, 0,])
            .is_err()
    );
    let mut delegate_v3 = vec![equinox::instruction::DELEGATE_V3_ACCOUNT, 1, 7];
    delegate_v3.extend_from_slice(&[9; 32]);
    assert!(matches!(
        EquinoxInstruction::decode(&delegate_v3),
        Ok(EquinoxInstruction::DelegateV3Account { kind: 1, index: 7, validator }) if validator == [9; 32]
    ));
}

/// A `TypeScript -> Rust` golden vector for `UpdateExchangeConfig`, encoded
/// exactly as `clients/equinox/src/index.ts::updateExchangeConfig`
...
/// `clients/equinox/src/index.test.ts`): only `makerFeeBps`,
/// `takerFeeBps`, and `keeperAuthority` are set.
#[test]
fn typescript_update_exchange_config_vector_decodes_in_rust() {
    use equinox::instruction::exchange_config_field as field;
    let mut data = vec![0u8; 196];
    data[0] = equinox::instruction::UPDATE_EXCHANGE_CONFIG;
    ...
    match EquinoxInstruction::decode(&data) {
        Ok(EquinoxInstruction::UpdateExchangeConfig {
