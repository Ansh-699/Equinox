...
fn fixture() -> Fixture {
    ...
    .unwrap();
    {
        ...
        let start = equinox::state::TRADER_SEAT_OFFSET
            + SEAT_INDEX as usize * equinox::state::TRADER_SEAT_SIZE;
        let seat =
            unsafe { &mut *(bytes.as_mut_ptr().add(start) as *mut equinox::state::TraderSeat) };
...
fn authorize_rejects_a_pre_funded_system_owned_session_pda() {
    ...
    assert_eq!(
        err_code(result),
        EquinoxError::InvalidTradingSession as u32
    );
}
...
fn duplicate_authorization_is_rejected() {
    ...
    // Same PDA, still simulated as Equinox-owned from the first call.
...
fn nonce_replay_lower_and_future_are_all_rejected_and_failed_actions_preserve_it() {
    ...
    assert_eq!(
        ...
        equinox::error::EquinoxError::SessionNonceReplay as u32
    );
    assert_eq!(
...
fn nonce_overflow_is_rejected() {
    ...
    );
    {
        ...
        equinox::session::write_session(bytes, &s).unwrap();
    }
    let signer_account = account(signer, Address::default(), 0, true, false);
    let mut accounts = [
        f.market.view.clone(),
...
fn reduce_only_close_action_permits_only_reduce_only_orders() {
    ...
    assert_eq!(
        err_code(result),
        equinox::error::EquinoxError::RiskViolation as u32
    );
}
...
fn cancel_order_consumes_nonce_and_no_notional() {
    ...
    .unwrap();
    let order_key = equinox::book::OrderInput {
        side: equinox::book::Side::Bid,
        tree: equinox::book::TreeKind::Fixed,
        ...
        peg_limit: 0,
        client_order_id: 1,
        time_in_force: equinox::book::TimeInForce::GoodTilCancelled,
        post_only: true,
        self_trade_behavior: equinox::book::SelfTradeBehavior::AbortTransaction,
    }
    .leaf()
    .unwrap()
    .key;
...
fn replace_order_succeeds_and_loses_time_priority() {
    ...
    .unwrap();
    let old_key = equinox::book::OrderInput {
        side: equinox::book::Side::Bid,
        tree: equinox::book::TreeKind::Fixed,
        owner: SEAT_INDEX as u32,
        price_or_offset: 100,
...
fn derive_trading_session_golden_vector_for_cross_language_parity() {
    ...
    // client's `deriveTradingSession` in `clients/equinox/src/index.test.ts`
