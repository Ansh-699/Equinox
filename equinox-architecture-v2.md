# Equinox — Updated Architecture and Product Experience
...
Equinox is a browser-first perpetual-futures exchange for tokenized equities on Solana. Users deposit USDC, then take long or short exposure to stocks such as Apple, Tesla, Nvidia, or broad-market ETFs. Orders are matched through an onchain central limit order book rather than against a conventional centralized matching server.
...
> Tokenized stocks provide spot ownership on Solana, but holders still lack a simple way to hedge downside, gain short exposure, or discover transparent onchain liquidity. Equinox combines a low-latency stock-perpetual CLOB with an equity-aware DBC Launch Lab, USDC collateral, MagicBlock execution, and Privy onboarding.
...
Equinox should make three promises:
...
```mermaid
flowchart TB
    U[Trader / Issuer / Agent Owner] --> UI[Equinox Next.js Web App]
    UI --> PRIVY[Privy: login + embedded Solana wallet]

    subgraph PERPS[Perps Exchange: Equinox core]
        direction LR
...
Equinox is intentionally composed of three planes with one-way data boundaries:
...
The OpenBook-derived inner-node layout relevant to Equinox is:
...
Manifest's shared-node Hypertree is sophisticated and memory-conscious, but red-black insertion and deletion require rotations, parent maintenance, and color invariants. Equinox instead adopts the Serum/OpenBook prefix-length trie because its 128-bit price-time key, separate fixed/pegged roots, and expiry cache already match the required order semantics.
...
| Pyth `TradingStatus` | Pyth `MarketSession` | Equinox mode | New exposure | Reduce/close | Pegged orders |
| --- | --- | --- | --- | --- | --- |
...
sequenceDiagram
    ...
    participant UI as Equinox Launch Lab
    ...
    participant Terminal as Equinox Terminal
...
ClawPump is a connector, not a custody or trading authority for Equinox. The application first queries the current ClawPump launch/pair capability. Only if the selected stock quote mint and launch mechanism are supported does it enable the agent flow. Otherwise the issuer can use direct Meteora DBC creation and the UI clearly marks the ClawPump bounty path unavailable.
...
- Equinox never receives a seed phrase.
...
Equinox should emphasize six differentiators:
...
| OpenBook `BookSide` concrete size is asserted | OpenBook v2 | Same `bookside.rs`, `const_assert_eq!(size_of::<BookSide>(), 90944)`; Equinox's 90,640-byte arena budget excludes its own surrounding market header/other regions and must be independently asserted |
...
These sources support the architecture; they are not permission to copy code. OpenBook v2 contains mixed MIT/GPL considerations, Manifest is GPL-3.0, and every implementation source must receive a license review before reuse. Equinox's Pinocchio implementation should reproduce required behavior from a clean specification unless the selected source license is intentionally accepted.
