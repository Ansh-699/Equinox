  ...
  const fetcher: typeof fetch = async (input: unknown, init?: RequestInit) => {
    ...
    if (body.method === "getMultipleAccounts") {
      return Response.json({
        ...
        result: { context: { slot: 1 }, value: [{ data: [Buffer.from(HEADER_BYTES).toString("base64"), "base64"], owner: "8Ucdsd3ejSEFFTpUivfK84eZv2q6aAe83A9zwSBcxFZ", lamports: 1_000_000_000, executable: false }] },
      });
    }
  ...
  it("one-market failure isolation + bounded work: a corrupt market account is a discovery error, not a crash", async () => {
    ...
    const fetcher: typeof fetch = async (input: unknown, init?: RequestInit) => {
      ...
      if (body.method === "getMultipleAccounts") {
        ...
        return Response.json({
          jsonrpc: "2.0", id: body.id,
          result: { context: { slot: 1 }, value: [{ data: [Buffer.from(corrupt).toString("base64"), "base64"], owner: "8Ucdsd3ejSEFFTpUivfK84eZv2q6aAe83A9zwSBcxFZ", lamports: 1, executable: false }] },
        });
      }
      return Response.json({ jsonrpc: "2.0", id: body.id, result: null });
    };
