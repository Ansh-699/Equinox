...
describe("V3 worker shard aggregation", () => {
  it("rejects a correctly-shaped shard owned by a foreign program", async () => {
    ...
    const transport = { multipleAccounts: async () => ({ context: { slot: 1 }, value: bytes.map((data, index) => ({ data: [btoa(String.fromCharCode(...data)), "base64"] as [string, string], owner: index === 0 ? "foreign-program" : "Gc4shx8j29nSuP4xATiKszBMZpzVEzc72Tr5iYwLALzZ", lamports: 1 })) }) } as never;
