...
import { decodeOracleSnapshotV3, deriveV3ExecutionAccounts } from "@/clients/equinox/src";
...
import { EquinoxProtocolService } from "@/lib/protocol-service";
...
export interface EquinoxProtocol {
  ...
  service: EquinoxProtocolService;
}
...
export function useEquinoxProtocol(marketAddress: string | null, override?: ActiveWalletSigner | null, market: V3Market = PRIMARY_MARKET): EquinoxProtocol | null {
  const walletSigner = useActiveWalletSigner();
  const signer = override ?? walletSigner;
  return useMemo(() => createEquinoxProtocol(signer, marketAddress, market), [signer, marketAddress, market]);
}

/** The protocol for one market and one signer (a page trading several markets makes one each). */
export function createEquinoxProtocol(signer: ActiveWalletSigner, marketAddress: string | null, market: V3Market = PRIMARY_MARKET): EquinoxProtocol | null {
  ...
  const marketLookupTables = () => lookupTable ??= market.lookupTable
    // Owned by the address-lookup-table program, so read it without the Equinox owner check.
  ...
  const freshOracle = snapshot && publicMarketApiUrl
    ...
    : undefined;
  return { walletAddress, rpc, router, service: new EquinoxProtocolService({ encode, wallet: signer, l1: rpc, er, freshOracle }) };
}
