# Equinox Devnet Release Gate

Status: NOT READY FOR DEVNET DEPLOYMENT

This gate records standalone Equinox evidence only. No devnet deployment
...
| --- | --- | --- |
| Independent workspace | Root Cargo workspace contains only `programs/equinox`. | PASS |
...
| SBF verification | `cargo build-sbf --manifest-path programs/equinox/Cargo.toml --features bpf-entrypoint` completed successfully. | PASS |
| Artifact evidence | `target/deploy/equinox.so`; SHA-256 `228aa1c9f1d1ac45494f9a118e3efced1695cc05b169c9f14b6cadcd743c5314`; 49,232 bytes. | PASS |
...
`docs/custody.md` and `docs/equinox-build-record.md`): a stale
...
(`docs/equinox-build-record.md`'s Layout Formalization entry). Worker-
...
(the compiled `equinox::ID` constant, `.env.example`, `clients/.../constants.ts`)
does not match any keypair material available on this machine:

| Keypair | Derived address |
| --- | --- |
| `.keys/equinox-program-keypair.json` (600 perms, gitignored) | `6QyZWQw7dvFNXerNdhzhyqQjzZnkMXmr52GNJLT1KpmU` |
| `target/deploy/equinox-keypair.json` (build default) | `gkPwvyYwDa7Ed5JkfvSGh2RPeakVYstFm5HEcWEZAqo` |
...
- Every canonical reference updated: Rust `equinox::ID` raw-byte
  constant, TypeScript `EQUINOX_PROGRAM_ID`, Worker keeper allowlist,
...
program address to the adjacent `target/deploy/equinox-keypair.json`
