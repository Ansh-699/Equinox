...
| V3 committable account lifecycle | Source-complete execution slice; setup/delegation/shard-commit live; core restore pending | The current MagicBlock scheduler source enforces `MAX_PERMITTED_DATA_INCREASE = 10,240`; V3 therefore uses 10,184-byte pages (115 nodes/page, 9 pages/side), not the superseded 22,592-byte/four-page design. Fresh Devnet V3 state is live at core `47Mx7SZvt7EY6NydsA5krgrqvcDDR1H5BG5xTPDSnhso` with 18 book + 4 seat + 4 event shards, all exact-sized and delegated to the MagicBlock delegation program; setup/delegation signatures are in `/tmp/opencode/v3-lifecycle-state.json` and `/tmp/opencode/v3-delegation-state.json`. The corrected program was upgraded repeatedly while fixing the deployed callback/commit path; the final deployed ELF is verified byte-for-byte by `scripts/verify-deployment.mjs`. A full 27-account intent is rejected by the validator with `0xa0000002`; the implemented fallback commits each of 26 children as a one-account intent and the core last, all ER submissions finalized. Evidence and checkpoint paths are in `docs/status/v3-sharded-commit-evidence-20260920.json` and `/tmp/opencode/v3-sharded-commit-state.json`. Child undelegations also finalized, but the core's first external callback was sent while the old parser rejected the shorter V3-core seed payload (`0x6012`); the parser fix is deployed, while that already-pending callback remains un-retried by the validator. Place/cancel/cancel-all and main-wallet replace remain locally tested only; session replace, V3 L1 custody/deposit/withdraw, funding/oracle ingestion and full cross-tree/self-trade policy remain incomplete. |
