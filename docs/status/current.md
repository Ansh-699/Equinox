...
| `clients/stockstream/src/index.ts` facade reduction | In progress | commits `7fba664`, `309a475`, `bc917d9`, `8f981f8`, `ddf3776`, `ac88ed9`, and `04dbf8f` remove duplicated opcode/state authorities, correct V3 session-replace encoding, and extract V3 order/cancel/funding/oracle/commit/recovery/custody builders into `abi/v3-instructions.ts`; `abi/encoding.ts` and `abi/transaction.ts` own shared integer, public-key, account-meta, and transaction primitives. Legacy V2 and lifecycle-specific encoders remain in the facade and require further parity-tested extraction. |
