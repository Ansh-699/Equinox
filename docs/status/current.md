...
| `clients/stockstream/src/index.ts` facade reduction | In progress | commits `7fba664`, `309a475`, and `bc917d9` remove duplicated opcode/state authorities and correct V3 session-replace encoding; `abi/encoding.ts` and `abi/transaction.ts` now own shared integer, public-key, account-meta, and transaction primitives. Instruction-specific encoders remain in the legacy facade and require further parity-tested extraction. |
