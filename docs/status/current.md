...
|---|---|---|
| ABI authority/parity | Partial | `npm run check:stockstream-abi` -> `ABI-OK`; commit `7fba664` makes the public `STOCKSTREAM_INSTRUCTION` compatibility export a direct re-export of `abi/instructions::OPCODE`, removing one duplicated 51-opcode authority. Event sizes, names, ABI version and `NO_SEAT` now come directly from `abi/events.ts` through the facade (`c115cd4`); event and legacy-book decoders are now direct canonical-module exports (`cc329b8`); the instruction-name decoder likewise moved to `abi/instructions.ts` (`7fcb56c`). The legacy market-state decoder and region checks now live in `abi/accounts.ts` and are re-exported by the facade, preserving the V2 compatibility API (`142e3cc`). TradingSession decoding now delegates through the canonical ABI module while preserving the PublicKey compatibility shape (`da290f7`). V3 seat-position decoding is now shared by client ABI and Worker (`daa46f0`, `1642137`); V3 custody opcodes 53/54 now have Rust decoder and TypeScript golden coverage. `clients/stockstream/src/index.ts` still owns most instruction construction and is not yet a thin facade. |
