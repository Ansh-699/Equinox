...
| `clients/stockstream/src/index.ts` facade reduction | In progress | commits `7fba664`, `309a475`, `bc917d9`, `8f981f8`, `ddf3776`, `ac88ed9`, `04dbf8f`, `9576c85`, `cc799d7`, `06965ee`, and `bd9eef5` remove duplicated opcode/state authorities, correct V3 session-replace encoding, and extract V3 order/cancel/funding/oracle/commit/recovery/custody/account-creation/initialization/delegation/session/registry/V2-custody builders into `abi/v3-instructions.ts`, `abi/session-instructions.ts`, `abi/registry-instructions.ts`, and `abi/custody-instructions.ts`; `abi/encoding.ts` and `abi/transaction.ts` own shared integer, public-key, account-meta, and transaction primitives. Legacy V2 execution builders remain in the facade and require further parity-tested extraction. |
