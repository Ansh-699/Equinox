...
| Worker deployment | Complete | `https://stockstream-market-api.ansht.workers.dev`; real D1 database (`1dced396-c76a-4147-8a4f-70465e9aff55`, 7 migrations applied); `/v1/health/keepers` reports `signer: ready`, `magicRouter: ready`. V3 aggregation now decodes complete persisted event records (commit `a1f48af`) while retaining strict 17-account and shard-relationship validation. |
...
| V3 committable account lifecycle | Partially source-complete; creation/activation SBF-runtime tested; not deployed | commits `7c91e31`, `8ffa31c`, `f037cc2`, `7e830d2`, `1148fbe`, `83a7d29`, `51d849c`, `7eb6a95`, `c6db456`, `5482c9a`, `1434629`, `6004cd4`: distinct V3 core/book/seat/event PDAs all remain below 50,000 bytes; opcode 46 creates/resumes them without accepting a V2 PDA; opcode 47 binds a core once to the exchange listing authority; opcode 48 delegates a bounded core/page/shard with real Delegation Program wire encodings; opcodes 49/50 create and safely close sharded seats with global duplicate-owner checks and V3 event-sequence/log parity. `PagedBookV3` now preserves 32-bit global node handles across four 256-node pages, fixed/pegged roots, free-list reuse and expiry pruning, with cross-page tests. Event shards store 32 complete 100-byte ABI records (3,244 bytes/account) and have validated sequence-to-shard append coverage. LiteSVM exercised creation/activation and a book page across 10,240 -> 20,480 -> 22,592 bytes; V3 matching, custody, commit scheduling and Devnet deployment are still outstanding. |
