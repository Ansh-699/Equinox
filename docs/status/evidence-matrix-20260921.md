...
This matrix is scoped to current HEAD `e132845` (post-V3-cancellation-atomicity, funding-settlement, liquidation-preflight, and cancel-all-preflight fixes). “Fresh” means rerun on this checkout; live claims additionally require the cited Devnet artifact. The full local gate is `VERIFY_SUMMARY_PATH=/tmp/stockstream-verify-summary-20260921-funding.json NO_DNA=1 bash scripts/verify.sh` from the preceding source checkpoint; the checked-in summary is `verify-latest.json`. The focused Rust suite and SBF build were rerun after the source fixes.
