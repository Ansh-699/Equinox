...
| replace accounting | locally-tested | `programs/stockstream/src/v3.rs`; `cargo test -p stockstream --test v3_bundle v3_session_replace_consumes_one_nonce_and_requires_replace_permission` verifies successful nonce consumption and over-limit replacement rejection with original page/session bytes unchanged | yes |
