...
import { MAGICBLOCK_DELEGATION_PROGRAM_ID, MAGICBLOCK_MAGIC_CONTEXT_ID, MAGICBLOCK_MAGIC_PROGRAM_ID, EQUINOX_PROGRAM_KEY, cancelAllV3, cancelOrderV3, placeOrderV3, replaceOrderV3, commitV3Shard, consumeOracleUpdateV3, updateFundingV3 } from "./index";
import { authorizeTradingSession, authorizeTradingSessionV3, cancelOrder, closeV3TraderSeat, commitMarket, delegateClusterMember, delegateV3Account, deriveClusterMemberPdas, createPerpMarket, createV3Account, createV3TraderSeat, decodeFillPayload, decodeInstruction, decodeMarketState, decodeSeatAmountPayload, decodeEquinoxEvent, decodeTradingSession, delegateMarket, deriveTradingSession, depositCollateral, depositCollateralV3, EVENT_SIZE, initializeExchange, initializeMarket, initializeV3Market, initializeVault, placeOrder, previewPlaceOrder, recordBadDebt, reconcileVault, registerStockInstrument, resolveBadDebt, transferToInsuranceFund, updateExchangeConfig, updateStockInstrument, updateV3Risk, withdrawCollateral, withdrawCollateralV3, withdrawInsuranceFunds, withdrawProtocolFees, EXCHANGE_CONFIG_FIELD, requestV3Undelegation, rollbackV3Undelegation } from "./index";
import { EQUINOX_ACCOUNT_SIZE } from "./constants";
...
test("V3 funding builder uses the canonical 27-account execution bundle", () => {
  const core = PublicKey.unique();
  const bookPages = Array.from({ length: 18 }, () => PublicKey.unique());
  const seatShards = Array.from({ length: 4 }, () => PublicKey.unique());
  const eventShards = Array.from({ length: 4 }, () => PublicKey.unique());
  const ix = updateFundingV3({ core, bookPages, seatShards, eventShards, authority }, 12n, 99n);
...
test("instruction constructors use canonical program id and exact account flags", () => {
  ...
  expect(ix.programId.toBase58()).toBe(EQUINOX_PROGRAM_ID);
...
test("transaction preview is unsigned and explicit about unavailable margin", () => {
  ...
  expect(preview.programId).toBe(EQUINOX_PROGRAM_ID);
...
test("integration constructors preserve discriminators, account order and signer flags", () => {
  ...
  expect(memberIx.keys.map((key) => [key.pubkey.toBase58(), key.isSigner, key.isWritable])).toEqual([
    ...
    [EQUINOX_PROGRAM_KEY.toBase58(), false, false],
  ]);
...
test("trading session PDA derivation matches the Rust program byte-for-byte", () => {
  ...
  // programs/equinox/tests/trading_session.rs
  ...
  expect(Array.from(pda.toBytes())).toEqual([
    253, 37, 117, 253, 187, 253, 34, 204, 68, 211, 185, 165, 180, 159, 46, 186, 139, 251, 115, 194,
    79, 141, 152, 94, 218, 173, 214, 96, 110, 151, 255, 233,
  ]);
});
...
function marketHeaderFixture(version: number): Uint8Array {
  const data = new Uint8Array(EQUINOX_ACCOUNT_SIZE);
...
test("decodeMarketState golden vector: custody-ledger byte offsets match the Rust layout and version 1 is rejected", () => {
  ...
  // `programs/equinox/tests/account_settlement.rs::custody_ledger_field_offsets_match_the_typescript_decoder`
  ...
  expect(() => decodeMarketState(marketHeaderFixture(1))).toThrow(/Invalid Equinox market header/);
});
...
test("decodeEquinoxEvent parses a Program data line using the binary event ABI", () => {
  ...
  const event = decodeEquinoxEvent(line);
  ...
  expect(decodeEquinoxEvent("Program log: not an event")).toBeNull();
  expect(decodeEquinoxEvent(`Program data: ${Buffer.alloc(10).toString("base64")}`)).toBeNull();
});

test("decodeEquinoxEvent preserves an unrecognized future discriminator instead of dropping it", () => {
  const market = Buffer.alloc(32, 0x01);
  const bytes = encodeEventFixture(9999, 1n, market, 0n, Buffer.alloc(48));
  const event = decodeEquinoxEvent(`Program data: ${bytes.toString("base64")}`);
...
test("decodeEquinoxEvent distinguishes OrderFilled from OrderPartiallyFilled sharing the fill payload shape", () => {
  ...
  const full = decodeEquinoxEvent(`Program data: ${encodeEventFixture(204, 10n, market, 0n, fillPayload).toString("base64")}`);
  ...
  const partial = decodeEquinoxEvent(`Program data: ${encodeEventFixture(203, 11n, market, 0n, fillPayload).toString("base64")}`);
  expect(partial?.kind).toBe("OrderPartiallyFilled");
  expect(partial?.discriminator).not.toBe(full?.discriminator);
});

test("decodeEquinoxEvent decodes the newly-wired session, liquidation, and MagicBlock event kinds", () => {
