...
export function delegateMarket(accounts: DelegationAccounts, validator: AddressInput): TransactionInstruction {
  ...
  const [buffer] = PublicKey.findProgramAddressSync([Buffer.from("buffer"), market.toBuffer()], EQUINOX_PROGRAM_KEY);
  ...
  return instruction(data, [accountMeta(market, false, true), accountMeta(accounts.authority, true, false), accountMeta(accounts.instrument, false, false), accountMeta(accounts.payer, true, true), accountMeta(buffer, false, true), accountMeta(delegationRecord, false, true), accountMeta(delegationMetadata, false, true), accountMeta(MAGICBLOCK_DELEGATION_PROGRAM_ID, false, false), accountMeta(SystemProgram.programId, false, false), accountMeta(EQUINOX_PROGRAM_KEY, false, false), ...(accounts.clusterAccounts ?? []).map((a) => accountMeta(a, false, true))]);
}
...
export function delegateClusterMember(accounts: ClusterMemberAccounts, validator: AddressInput): TransactionInstruction {
  ...
  const [buffer] = PublicKey.findProgramAddressSync([Buffer.from("buffer"), member.toBuffer()], EQUINOX_PROGRAM_KEY);
  ...
  return instruction(data, [accountMeta(market, false, false), accountMeta(accounts.authority, true, false), accountMeta(member, false, true), accountMeta(buffer, false, true), accountMeta(delegationRecord, false, true), accountMeta(delegationMetadata, false, true), accountMeta(accounts.payer, true, true), accountMeta(MAGICBLOCK_DELEGATION_PROGRAM_ID, false, false), accountMeta(SystemProgram.programId, false, false), accountMeta(EQUINOX_PROGRAM_KEY, false, false)]);
}
