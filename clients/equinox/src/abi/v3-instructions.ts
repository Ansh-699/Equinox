...
export function requestV3Undelegation(accounts: V3UndelegationRecoveryAccounts): TransactionInstruction {
  return instruction(Uint8Array.of(OPCODE.requestV3Undelegation), [
    ...
    accountMeta(EQUINOX_PROGRAM_KEY, false, false), accountMeta(accounts.request, false, true),
...
export function rollbackV3Undelegation(accounts: V3UndelegationRecoveryAccounts): TransactionInstruction {
  ...
  return instruction(Uint8Array.of(OPCODE.rollbackV3Undelegation), [
    accountMeta(accounts.core, false, true), accountMeta(EQUINOX_PROGRAM_KEY, false, false),
...
/** Per-(market, trader) deposit receipt used by the inbox (programs/equinox/src/inbox.rs). */
export function deriveDepositReceiptV3(core: AddressInput, trader: AddressInput): PublicKey {
  return PublicKey.findProgramAddressSync([Buffer.from("deposit-receipt-v3"), publicKey(core).toBuffer(), publicKey(trader).toBuffer()], EQUINOX_PROGRAM_KEY)[0];
}
...
export function deriveWithdrawReceiptV3(core: AddressInput, trader: AddressInput): PublicKey {
  return PublicKey.findProgramAddressSync([Buffer.from("withdraw-receipt-v3"), publicKey(core).toBuffer(), publicKey(trader).toBuffer()], EQUINOX_PROGRAM_KEY)[0];
}
...
export function delegateV3Account(accounts: V3DelegationAccounts, kind: V3AccountKind, validator: AddressInput, index = 0): TransactionInstruction {
  ...
  const [kindByte, flattenedIndex] = v3KindAndIndex(kind, index);
  if (!target.equals(expectedV3Account(parent, kindByte, flattenedIndex))) throw new RangeError("target is not the derived V3 account PDA");
  const [buffer] = PublicKey.findProgramAddressSync([Buffer.from("buffer"), target.toBuffer()], EQUINOX_PROGRAM_KEY);
  ...
  return instruction(data, [accountMeta(parent, false, false), accountMeta(target, false, true), accountMeta(accounts.authority, true, false), accountMeta(accounts.payer, true, true),
    accountMeta(buffer, false, true), accountMeta(record, false, true), accountMeta(metadata, false, true), accountMeta(MAGICBLOCK_DELEGATION_PROGRAM_ID, false, false), accountMeta(SystemProgram.programId, false, false), accountMeta(EQUINOX_PROGRAM_KEY, false, false)]);
}
