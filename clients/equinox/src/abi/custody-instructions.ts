...
export function withdrawCollateral(accounts: CustodyAccounts, amount: bigint | number): TransactionInstruction {
  ...
  const vaultAuthority = PublicKey.findProgramAddressSync([Buffer.from("vault-authority"), new PublicKey(accounts.market).toBuffer()], new PublicKey(EQUINOX_PROGRAM_ID))[0];
