...
export function decodeMarketState(bytes: Uint8Array): MarketStateView {
  if (bytes.byteLength !== EQUINOX_ACCOUNT_SIZE) throw new RangeError("Invalid Equinox market account size");
  ...
  if (!header) throw new RangeError("Invalid Equinox market header");
  if (header.bidArenaOffset !== BID_ARENA_OFFSET || header.askArenaOffset !== ASK_ARENA_OFFSET
    ...
    throw new RangeError("Invalid Equinox regions");
  }
  return {
