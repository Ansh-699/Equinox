...
# Usage: scripts/preipo-market.sh <SYMBOL> <feed-id>   e.g. OPENAI 4000000001
# Signs with the market authority (~/.config/solana/id.json) -- run locally only.
set -euo pipefail
cd "$(dirname "$0")/.."
symbol="${1:?symbol}"; feed="${2:?feed id}"
...
export V3_INSTRUMENT_ID_HEX="$(python3 -c "print(b'PRESTOCKS:$symbol'.ljust(32, b'\0').hex())")"
export V3_ORACLE_FEED_ID="$feed" V3_ORACLE_SYMBOL="PreStocks.$symbol/USD" V3_ORACLE_CHANNEL=fixed_rate@1000ms V3_ORACLE_EXPONENT=-5
