#!/usr/bin/env bash
# Installs and verifies the pinned Equinox SBF toolchain.
#
# The single source of truth for versions is toolchain/equinox-runtime.env.
...
# shellcheck source=../toolchain/equinox-runtime.env
source "$ROOT/toolchain/equinox-runtime.env"
export PATH="$HOME/.local/share/solana/install/active_release/bin:$PATH"

if ! command -v agave-install >/dev/null 2>&1; then
  curl --fail --location --proto '=https' --tlsv1.2 \
    "https://release.anza.xyz/v${AGAVE_VERSION}/install" \
    -o /tmp/equinox-agave-install.sh
  sh /tmp/equinox-agave-install.sh
fi
...
cargo build-sbf --manifest-path "$ROOT/programs/equinox/Cargo.toml" --features bpf-entrypoint
python3 "$ROOT/scripts/verify-sbf-artifact.py" "$ROOT/target/deploy/equinox.so"
