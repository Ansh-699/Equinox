...
export const DEMO_LOOKUP_TABLE: string = process.env.NEXT_PUBLIC_STOCKSTREAM_LOOKUP_TABLE
  ?? (deployment as { lookupTable?: string | null }).lookupTable ?? "";
/** L1 OracleSnapshotV3 (authenticated Pyth price) read by the terminal. */
export const DEMO_ORACLE_SNAPSHOT: string = process.env.NEXT_PUBLIC_STOCKSTREAM_ORACLE_SNAPSHOT
