...
import { EQUINOX_ACCOUNT_SIZE, EQUINOX_PROGRAM_ID, EQUINOX_TRADING_SESSION_SIZE } from '../clients/equinox/src/constants';

const marketAddress = '8Ucdsd3ejSEFFTpUivfK84eZv2q6aAe83A9zwSBcxFZ';
function sessionBytes({ revoked = false } = {}) {
  const bytes = Buffer.alloc(EQUINOX_TRADING_SESSION_SIZE);
  ...
  new PublicKey(EQUINOX_PROGRAM_ID).toBuffer().copy(bytes, 76); // targetProgram
...
function marketBytes(commit = 4n, event = 7n) {
  const bytes = Buffer.alloc(EQUINOX_ACCOUNT_SIZE);
...
describe('production RPC transports', () => {
  ...
  it('simulates, submits once, confirms, and validates the market owner', async () => {
    ...
    const fetcher: typeof fetch = async (_input, init) => {
      ...
      if(request.method==='getAccountInfo') return response(request.id,{value:{owner:EQUINOX_PROGRAM_ID,executable:false,data:[marketBytes().toString('base64'),'base64']}});
  ...
  it('router requires one delegated validator and confirms ER acceptance', async () => {
    ...
    const rpc=new SolanaRpcTransport('https://router.test',async(_input,init)=>{
      ...
      if(body.method==='getAccountInfo') return response(id,{value:{owner:EQUINOX_PROGRAM_ID,executable:false,data:[marketBytes().toString('base64'),'base64']}});
  ...
  it('reads back an authorized trading session for readback-gated enablement', async () => {
    const rpc=new SolanaRpcTransport('https://rpc.test',async(_input,init)=>{
      const body=JSON.parse(String(init?.body));
      return response(body.id,{value:{owner:EQUINOX_PROGRAM_ID,executable:false,data:[sessionBytes().toString('base64'),'base64']}});
    });
  ...
  it('rejects a session account not owned by the Equinox program', async () => {
